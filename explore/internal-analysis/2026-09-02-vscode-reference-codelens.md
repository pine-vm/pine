...
[provided screenshot](https://github.com/user-attachments/assets/d662971d-f59c-4772-8876-6c2c5174f757):
...
- Updated the feature screenshot to the latest supplied VS Code reference-peek
  image.
- Full CI exposed 17 failures missed by the focused feature tests. Fifteen were
  stale embedded Elm regression probes: after `DeclarationRange` gained its
  canonical-name range, those fixtures still partially applied the old
  two-argument constructor. The resulting function value was embedded in
  `LocationInFile`, producing the misleading Pine-expression blobs shown in the
  assertions. Supplying the third range fixed all 15 probes without changing
  compiler behavior.
- The other two failures were deterministic VM performance snapshots for
  document-symbol and challenging rename scenarios. Their semantic response
  assertions passed; the snapshots were refreshed to include the additional
  canonical-range construction work.
