...
## 2026-09-13 — Graph-carried target discovery

Added immutable value facts through lists, projections, safe builtin transfers and explicit graph edges; the existing bounded inliner now consumes proven encoded targets. Analysis preserves all executable operations and never erases operand failures. Runtime and instruction interfaces remain unchanged.

- Early checkpoint `232818e9`: standalone bounded graph-value analysis and **32 passing tests**, including actual frontend head/skip lowering, matching/conflicting joins, invocation captures and unknown return slots. Scoped formatting passed. Log: `/home/runner/work/super-duper-disco/super-duper-disco/implement/Pine.Core.Tests/artifacts/test-logs/Pine.Core.Tests/2026-09-13T16-07-44_filtered.log`.
...
- Analysis is lazy, reused only for the current graph version, and charged against a cumulative budget independently of parse memo warmth. A successful inline invalidates facts and retries previously unknown call targets: a returned function target may become provable only after its defining invocation disappears.
- Actual compiled Alfa with runtime source/offset matches direct interpretation on all 13 cases. Three graph expansions reduce aggregate invocations **42 → 19** versus the unoptimized **graph** wrapper, not versus the existing optimized production compiler. Lists remain **2,139 → 2,139**; instructions **40,305 → 40,259**; layout-sensitive loop counters **168 → 168**. The explicit larger test policy exhausts its 1,000,000-unit analysis budget safely; residual recursive calls are not converted to loops. This demonstrates usage-site target discovery/inlining, not completed Alfa allocation elimination or production cutover.

Validation: scoped formatting; the established regression gate reports **545 passed, one existing skip**. Log: `/home/runner/work/super-duper-disco/super-duper-disco/implement/Pine.Core.Tests/artifacts/test-logs/Pine.Core.Tests/2026-09-13T16-11-58_filtered.log`. The subsequent full-counter/no-extra-list assertion passed the targeted Alfa test (`2026-09-13T16-13-25_filtered.log` in the same directory). Independent reviews found no significant issues in either checkpoint; final automated validation pending.
...
| --- | --- |
| Would a demand-driven or worklist value analysis avoid repeated whole-graph refinement after each inline? | Measure analyzed definitions, work units and compile allocations on the closed-table Alfa wrapper; retain deterministic budget fallback and sound joins/backedges. The current 1,000,000-unit exhaustion is a concrete baseline, not evidence that runtime optimization is complete. |
