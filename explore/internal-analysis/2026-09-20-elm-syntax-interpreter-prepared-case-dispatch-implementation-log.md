...
- Added bottom-up preparation-time reduction of closed expressions to cached `ValueLiteral` nodes, including the closed `take` and `skip` subexpressions used to construct blob values in `Char.elm`.
...
- [x] Reduced closed expressions and nested closed subexpressions to fully represented literal values during module preparation
- [x] Added regression coverage for let/case reduction, top-level declaration references, and the `Char.elm` blob-building expressions
...
- Closed-expression reduction runs bottom-up after an initial non-reducing preparation pass has made every declaration available for resolution. It uses abstract-syntax free-variable analysis, the standard interpreter resolver, and a bounded trampoline evaluation; successful concrete results within the materialization limits are rebuilt with `CreateFullyRepresented`, while function-bearing, oversized, or failed evaluations retain their prepared expression form.
...
22. `dotnet run -- --filter-class="*PreparedSyntaxTests" --no-progress --no-ansi`
    - Result: Passed, 12 tests
    - Log: `artifacts/test-logs/Pine.Core.Tests/2026-09-20T13-36-48_filtered.log`

23. `dotnet run --no-build -- --filter-class="*CharBuiltinTests" --no-progress --no-ansi`
    - Result: Passed, 25 tests
    - Log: `artifacts/test-logs/Pine.Core.Tests/2026-09-20T13-37-32_filtered.log`

24. `dotnet run -- --filter-class="*PreparedSyntaxTests" --no-progress --no-ansi`
    - Result: Passed, 13 tests
    - Log: `artifacts/test-logs/Pine.Core.Tests/2026-09-20T13-59-02_filtered.log`

25. Focused `LetBlockExpressionsTests`, `CaseBlockTests`, `InfiniteRecursionTests`, and `ModuleImportTests`
    - Result: Passed, 58 tests total

26. `dotnet run --no-build -- --filter-class="*CharBuiltinTests" --no-progress --no-ansi`
    - Result: Passed, 25 tests
    - Log: `artifacts/test-logs/Pine.Core.Tests/2026-09-20T13-59-21_filtered.log`
...
- Syntactically closed expressions can evaluate to closures or partially applied constructors, which cannot be serialized as literal Pine values. The reducer therefore materializes only results without opaque function-bearing values and leaves the original prepared expression intact otherwise.
- Closed source expressions can diverge, intentionally fail, or create excessively large lazy values. Preparation-time evaluation is bounded to 100,000 trampoline iterations, 100,000 value nodes, and 16 MiB of blob data, and falls back to the prepared expression on an error or exception instead of changing module-preparation behavior.
