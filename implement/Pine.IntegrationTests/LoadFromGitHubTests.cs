...
public class LoadFromGitHubTests
{
    ...
    public void Test_LoadFromGitHub_Tree_at_root()
    {
        ...
        {
            loadedFilesNamesAndHashes.Should().Contain(
                expectedFileNameAndHash,
    ...
    public void LoadFromGitHub_Commits_Contents_And_Lineage()
    {
        ...
        loadFromGithubResult.firstParentCommitWithSameTree.content.message.Should().Be(
    ...
    public void LoadFromGitHub_URL_points_only_to_repository()
    {
        ...
        var loadedFilesPathsAndContents =
            loadFromGithubResult.tree.EnumerateFilesTransitive()
            .Select(
                blobPathAndContent => (filePath: string.Join("/", blobPathAndContent.path),
                fileContent: blobPathAndContent.fileContent))
            .ToImmutableList();
        ...
        try
        {
                ...
                LoadFromGitHubOrGitLab.GetRepositoryFilesPartialForCommitRequest request)
            {
                ...
                var httpRequest =
                    new HttpRequestMessage(
                        HttpMethod.Get,
                        requestUri:
                        serverUrl.TrimEnd('/') + GitPartialForCommitServer.ZipArchivePathFromCommit(request.commit))
                    {
...

            {
                ...
                Convert.ToHexStringLower(SHA256.HashData(blobContent.Span))
                    .ToLowerInvariant()
                    .Should().Be(
                    "e80817b2aa00350dff8f00207083b3b21b0726166dd695475be512ce86507238",
...

            {
                ...
                var loadFromGitHubResult =
                    LoadFromGitHubOrGitLab.LoadFromUrl(
                        sourceUrl:
                        "https://github.com/pine-vm/pine/blob/30c482748f531899aac2b2d4895e5f0e52258be7/azure-pipelines.yml",
                        getRepositoryFilesPartialForCommit:
                        request =>
                        Result<string, IImmutableDictionary<IReadOnlyList<string>, ReadOnlyMemory<byte>>>.ok(
                            ConsultServer(request)))
                ...
                Convert.ToHexStringLower(SHA256.HashData(blobContent.Span))
                    .ToLowerInvariant()
                    .Should().Be(
                    "a328195ad75edf2bcc8df48b3d59db93ecc19b95b6115597c282900e1cf18cbc",
                    "Loaded blob content hash equals expected hash.");

                stopwatch.Elapsed.TotalSeconds.Should().BeLessThan(
                    3,
                    "Reading another blob from an already cached commit should complete fast.");
            }
        }
        finally
        {
