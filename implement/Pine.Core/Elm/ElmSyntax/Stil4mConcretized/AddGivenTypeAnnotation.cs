...
public class SetTypeAnnotation
{
    ...
    private static Stil4mElmSyntax7.Node<T> MakeNode<T>(T value) =>
    ...
    {
        var updatedDeclarations = new List<Stil4mElmSyntax7.Node<Declaration>>();

        foreach (var declaration in fileBefore.Declarations)
        {
            var updatedDeclaration = UpdateDeclarationTypeAnnotations(
                declaration,
                declarationPath: [],
        ...
        return fileBefore with { Declarations = updatedDeclarations };
    }
        ...
        System.Func<IReadOnlyList<string>, TypeAnnotation?, TypeAnnotation?> declarationsTypeAnnotations)
    {
        ...
        Stil4mElmSyntax7.Node<Signature>? updatedSignature;
    ...
    private static Stil4mElmSyntax7.Node<Declaration> UpdateDeclarationTypeAnnotations(
        Stil4mElmSyntax7.Node<Declaration> declaration,
    ...
    private static Stil4mElmSyntax7.Node<Expression> UpdateExpressionTypeAnnotations(
        Stil4mElmSyntax7.Node<Expression> expression,
        ...
        System.Func<IReadOnlyList<string>, TypeAnnotation?, TypeAnnotation?> declarationsTypeAnnotations)
    {
        switch (expression.Value)
        {
            case Expression.LetExpression letExpr:
                var updatedDeclarations = new List<Stil4mElmSyntax7.Node<Expression.LetDeclaration>>();
                ...
                var updatedLetExpression = new Expression.LetExpression(updatedLetBlock);
            ...
            case Expression.IfBlock ifBlock:
                ...
                var updatedIfBlock = new Expression.IfBlock(
            ...
            case Expression.CaseExpression caseExpr:
                ...
                return expression with { Value = new Expression.CaseExpression(updatedCaseBlock) };
            ...
            case Expression.LambdaExpression lambdaExpr:
                var updatedLambdaBody = UpdateExpressionTypeAnnotations(
                    ...
                    parentPath,
                    declarationsTypeAnnotations);
                var updatedLambda = lambdaExpr.Lambda with { Expression = updatedLambdaBody };
                return expression with { Value = new Expression.LambdaExpression(updatedLambda) };

            case Expression.Application appExpr:
                var updatedArgs = appExpr.Arguments
                    .Select(arg => UpdateExpressionTypeAnnotations(arg, parentPath, declarationsTypeAnnotations))
                    .ToList();
                return expression with { Value = new Expression.Application(updatedArgs) };
            ...
            case Expression.ParenthesizedExpression parenExpr:
                var updatedInner = UpdateExpressionTypeAnnotations(
                    ...
                    parentPath,
                    declarationsTypeAnnotations);
                return expression with { Value = new Expression.ParenthesizedExpression(updatedInner) };
            ...
            case Expression.TupledExpression tupledExpr:
                ...
                return expression with { Value = new Expression.TupledExpression(updatedElements) };

            case Expression.ListExpr listExpr:
                var updatedListElements = UpdateSeparatedSyntaxList(
                    listExpr.Elements,
                    elem => UpdateExpressionTypeAnnotations(elem, parentPath, declarationsTypeAnnotations));
                return expression with { Value = new Expression.ListExpr(updatedListElements) };
            ...
            case Expression.RecordExpr recordExpr:
                ...
                return expression with { Value = new Expression.RecordExpr(updatedRecordFields) };
            ...
            case Expression.Negation negation:
                var updatedNegationExpr = UpdateExpressionTypeAnnotations(
                    ...
                    parentPath,
                    declarationsTypeAnnotations);
                return expression with { Value = new Expression.Negation(updatedNegationExpr) };
    ...
    private static Stil4mElmSyntax7.Node<Expression.LetDeclaration> UpdateLetDeclarationTypeAnnotations(
        ...
        System.Func<IReadOnlyList<string>, TypeAnnotation?, TypeAnnotation?> declarationsTypeAnnotations)
    {
        switch (letDecl.Value)
        {
            case Expression.LetDeclaration.LetFunction letFunc:
                ...
                return letDecl with
                {
                    Value = new Expression.LetDeclaration.LetFunction(updatedFunc)
                };
