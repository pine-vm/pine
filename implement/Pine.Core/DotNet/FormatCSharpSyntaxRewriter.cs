    ...
    : CSharpSyntaxRewriter
{
    ...
    public override SyntaxNode? VisitCompilationUnit(CompilationUnitSyntax originalNode)
    {
        ...
        if (node.Usings.Count > 0)
        {
            ...
            // Sort regular usings alphabetically.
            regularUsings = [.. regularUsings.OrderBy(u => u.Name?.ToString() ?? "")];
            staticUsings = [.. staticUsings.OrderBy(u => u.Name?.ToString() ?? "")];
    ...
    public override SyntaxNode? VisitCollectionExpression(CollectionExpressionSyntax originalNode)
    {
...

        {
            ...
            // align the close bracket with the enclosing expression level
            // (e.g., `])` aligns with `PineValue.List(` not with `[`).
            var closeBracketIndentTrivia =
                originalNode.Parent is ArgumentSyntax { Parent: ArgumentListSyntax argList }
                ?
                ComputeIndentationTriviaForNode(argList)
                :
                bracketIndentationTrivia;
    ...
    private static bool ExpressionCausesLineBreak(ExpressionSyntax expression) =>
        expression.DescendantTrivia().Any(t => t.IsKind(SyntaxKind.EndOfLineTrivia));
