...
/// (<see cref="Expression"/> <c>origExpr</c>, <see cref="StaticExpression"/> <c>body</c>, <see cref="PineValueClass"/> <c>constraint</c>).
...
public record StaticProgram(
    IReadOnlyDictionary<string, (Expression origExpr, StaticExpression body, PineValueClass constraint)> NamedFunctions);
