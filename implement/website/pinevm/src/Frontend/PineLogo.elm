module Frontend.PineLogo exposing (..)
...
pineLogoSvg : ShapeConfig -> { fill : Int -> String, svgAttributes : List (Svg.Attribute e) } -> Svg.Svg e
pineLogoSvg shape { fill, svgAttributes } =
    let
        logoPolygons : List ( Polygon2d.Polygon2d Quantity.Unitless coordinates, Int )
        logoPolygons =
            pineLogoShape shape

        allPoints =
            logoPolygons
                |> List.concatMap (Tuple.first >> Polygon2d.outerLoop)
                ...
                |> List.map
                    (\( polygon, lightness ) ->
                        ...
                        Svg.polygon
                            ...
                            , Html.Attributes.style "fill" (fill lightness)
                            ]
                            []
                    )
    in
...
pineLogoShape : ShapeConfig -> List ( Polygon2d.Polygon2d Quantity.Unitless coordinates, Int )
pineLogoShape config =
    let
        plainPolygons : List ( Polygon2d.Polygon2d Quantity.Unitless coordinates, Int )
        plainPolygons =
            pineLogoPolygons
                |> List.map
                    (Tuple.mapFirst
                        (List.map
                            (\( x, y ) ->
                                Point2d.unitless (toFloat x) (toFloat y)
                            )
    ...
    in
    plainPolygons
        |> List.map (Tuple.mapFirst (shrinkPolygon shrinkThickness))
...
pineLogoPolygons : List ( List ( Int, Int ), Int )
pineLogoPolygons =
