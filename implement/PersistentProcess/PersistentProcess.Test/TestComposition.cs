        ...
        public void Composition_from_file_tree()
        {
                    ...
                    expectedOutput = new Composition.Component
                    {
                        ListContent = new []
                        {
                            new Composition.Component
                            {
                                ListContent = new []
                                {
                                    new Composition.Component{ BlobContent = Encoding.UTF8.GetBytes("a").ToImmutableList()},
                            ...
                            new Composition.Component
                            {
                                ListContent = new []
                                {
                                    new Composition.Component{ BlobContent = Encoding.UTF8.GetBytes("b").ToImmutableList()},
                                    new Composition.Component
                                    {
                                        ListContent = new []
                                        {
            ...
            foreach (var testCase in testCases)
            {
                var asComposition = Composition.FromTree(
                    Composition.TreeFromSetOfBlobsWithCommonOSPath(testCase.input, Encoding.UTF8));
